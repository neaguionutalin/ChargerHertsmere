...
public class ChargerStatusService {
    ...
    public boolean changeStatus() {
        ...
        com.twilio.rest.api.v2010.account.Message message = com.twilio.rest.api.v2010.account.Message.creator(new com.twilio.type.PhoneNumber("+447490927845"), new com.twilio.type.PhoneNumber("+447588667442"), String.format("Phone is now %s.", this.makeCall ? "on" : "off")).create();
    ...
    public void checkStatus() throws JsonProcessingException, URISyntaxException {
        ResponseObject response = restTemplate.getForObject("https://charge.pod-point.com/ajax/pods/1545", ResponseObject.class);
        if (response == null) return;
        response.setPods(response.getPods().stream().sorted().peek(pod -> pod.setStatuses(pod.getStatuses().stream().sorted().toList())).toList());
        ...
        if (!Objects.equals(response, activeResponseObject) && this.makeCall) {
            ...
            Twilio.init(ACCOUNT_SID, AUTH_TOKEN);
            Call call = Call.creator(new PhoneNumber("+447490927845"), new PhoneNumber("+447588667442"), new URI("http://demo.twilio.com/docs/voice.xml")).create();
